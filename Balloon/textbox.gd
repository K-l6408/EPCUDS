...
@export var size_override = Vector2(154, 154)
@export var position_override = Vector2(0, 0)
...
func _process(_delta):
	balloon.custom_minimum_size = Vector2(154, 154)
	if Engine.is_editor_hint():
		balloon.size = size_override if override else Vector2(680, 200)
		balloon.position = position_override if override else Vector2(10, 290)
	else:
		balloon.size = size_override if override else Vector2(balloon.get_viewport_rect().size.x - 20, 200)
		balloon.position = position_override if override else Vector2(10, balloon.get_viewport_rect().size.y - balloon.size.y - 10)
	var resiz = max($Balloon/Margin.size.x / 5, responses_menu.size.x+70)
	$Balloon/ResponseBalloon.size = Vector2(resiz, responses_menu.size.y+70)
	$Balloon/ResponseBalloon.position = Vector2($Balloon/Margin.size.x - resiz,\
	-70-responses_menu.size.y)
	$Balloon/ResponseBalloon.visible = (not get_responses().is_empty()) and \
	is_equal_approx(dialogue_label.visible_ratio, 1)
